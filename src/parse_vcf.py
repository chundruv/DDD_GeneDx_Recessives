...
def variant_parser(args):
    ...
    Input args:
        ...
        unrelpar    =   unrelated parents list. Columns "ID1 ID2" (ID1 and ID2 same as above) 
        unaffectedpar   =   unaffected parents list. Columns "ID1 ID2" (ID1 and ID2 same as above) 
        ...
        gdxblacklist    =   BED file (UCSC) of blacklisted genes from GeneDx
        idmap   =   sample ID map file for DDD; required columns: 'person_stable_id', 'vcf_id'
    ...
    ped, parents, unaffected, samples_pop, samples_pop_inds, unrelated_parents_pop_index, populations = process_ped(args.pop, args.unaffectedpar, args.unrelpar, args.ped, samples)
    csq = extract_csq(x)
    
    id_map=pd.read_csv(args.idmap, sep=r'\s+', low_memory=False)
                    ...
                    if alt !="*":
                        ...
                        if len(hethoms)>0:
                            ...
                            for i in hethoms:
                                ...
                                if pop!='':            
                                    csvwrite.writerow(['NA' if x=='' else x for x in [get_stable_ids(id_map,i), [var if var!=3 else 'NA' for var in gt_types[np.where(np.isin(samples,i))]][0], i in ped,
                                        get_stable_ids(id_map,parents[i]) if i in parents else 'NA',
                                        [var if var!=3 else 'NA' for var in gt_types[np.where(np.isin(samples,parents[i]))]][0] if i in parents else 'NA',
                                        get_stable_ids(id_map,ped[i][0]) if i in ped else 'NA',[var if var!=3 else 'NA' for var in gt_types[np.where(np.isin(samples,ped[i][0]))]][0] if i in ped else 'NA',
                                        get_stable_ids(id_map,ped[i][1]) if i in ped else 'NA',[var if var!=3 else 'NA' for var in gt_types[np.where(np.isin(samples,ped[i][1]))]][0] if i in ped else 'NA',
