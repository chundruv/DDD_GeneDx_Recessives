...
def calc_obs(x, unrel_parents, unrel_probands, populations, consequence_classes, genes, probands_populations, chets):
    ...
    x=x[(np.isin(x['stable_id'], unrel_probands[0].tolist())) | (x['is_proband']==False)]
        ...
        for pop in populations:
            ...
            for c in consequence_classes:
                ...
                for i in range(0, x_subset.shape[0]):
                    if x_subset.iloc[i,0] not in probands_populations['person_stable_id'].tolist():
                        continue
                    if c!='synonymous/synonymous':
                        if x_subset.iloc[i,0] in prev_count:
                            continue
                    if x_subset.iloc[i, 1]==1:
                        if len(chets[(chets['stable_id']==x_subset.iloc[i,0]) & (chets['variant_id']==x_subset.iloc[i,13]) & (chets['functional_category']==c) & (chets['gene.stable.id']==gene)])>0:
                            if x_subset.iloc[i,0] not in chets_in_gene:
                                varIDs[c][gene][pop][x_subset.iloc[i,0]]={var:1 for var in chets[(chets['stable_id']==x_subset.iloc[i,0]) & (chets['functional_category']==c) & (chets['gene.stable.id']==gene)]['variant_id'].tolist()}
                                OB[c][gene][pop]+=1
